...
const ProductsCard = ({ product }) => {
  ...
  return (
    <div className='group relative mx-2 md:mx-0'>
        ...
        <div className="w-full border-[1px] px-2 py-4">
          <div className='flex justify-between items-center'>
            ...
            <div className='flex justify-end gap-2 relative overflow-hidden w-28 text-sm'>
              ...
              <p onClick={handleAddToCart} className='absolute z-20 w-[100px] text-gray-500 hover:text-gray-900 flex items-center gap-1 top-0 transform -translate-x-32 group-hover:translate-x-0 transition-transform cursor-pointer duration-500'>add to cart{" "}<span><BsArrowRight /></span></p>
            </div>
          </div>
          <div>
            <p>{product.category}</p>
          </div>
          <div className='absolute top-4 right-0 mr-1'>
            {product.isNew && (
